...
class DropDownSchedule(discord.ui.Select):
    def __init__(self, bot, ctx, datas, command, vals):
        ...
        for data in datas:
            ...
            options.append(discord.SelectOption(label=time, value=str(data[9])))
        super().__init__(
            max_values = 1,
            min_values = 1,
            options = options
    ...
    async def callback(self, inter):
        ...
        if inter.user.id != self.ctx.author.id:
            return await inter.followup.send("You cannot interact with this menu.", ephemeral=True)
                ...
                if data[3] == 1:
                    ...
                    if data[6] != "None":
                        try:
                            ...
                            pass
                    if data[7] != "None":
                        if isinstance(data[7], str) and (data[7].startswith("http://") or data[7].startswith("https://")):
                            try:
                    ...
                    if self.command == 'remove':
                        view = func.ConfirmationButtons(inter.user.id)
                        await inter.followup.send(content="Are you sure this is the schedule you want to delete?", embed=embed, view=view)
                        await view.wait()
                        if view.value:
                            await func.DataUpdate(self.bot, f"DELETE FROM schedules WHERE guild_id = {inter.guild.id} and num = {self.values[0]}")
                            for data in reversed(self.datas):
                        ...
                        view = func.ConfirmationButtons(inter.user.id)
                        await inter.followup.send(content="Are you sure this is the schedule you want to edit?", embed=embed, view=view)
                        await view.wait()
                        if view.value:
                    ...
                    if self.command == 'remove':
                        view = func.ConfirmationButtons(inter.user.id)
                        await inter.followup.send(content="Are you sure this is the schedule you want to delete?\n\n"+message, allowed_mentions=AllowedMentions.none(), view=view)
                        await view.wait()
                        if view.value:
                            await func.DataUpdate(self.bot, f"DELETE FROM schedules WHERE guild_id = {inter.guild.id} and num = {self.values[0]}")
                            for data in reversed(self.datas):
                        ...
                        view = func.ConfirmationButtons(inter.user.id)
                        await inter.followup.send(content="Are you sure this is the schedule you want to edit?\n\n"+message, allowed_mentions=AllowedMentions.none(), view=view)
                        await view.wait()
                        if view.value:
                            await EditCommandCheck(data)
                    else:
...
class SchedulerCommands(Cog):
    ...
    @add.command()
    async def message(self, ctx, channel: discord.TextChannel, duration, *, message):
    ...
    async def ScheduleAdd(self, ctx, channel, duration, description, embed, command="add"):
        if command == 'add':
            ...
            time = duration
        await ctx.send(embed=discord.Embed(title="Loop", description="Input how many times you want the schedule to loop. Type `0` for infinite loop. `1` will be taken in case of wrong input."))
        ...
        try:
            ...
            loop_integer = 1
        if embed == 0:
            await ctx.send(embed=discord.Embed(description="Now input the attachment you need with the message. Type `None` to skip it.", title="Attachment"))
            ...
            await func.DataUpdate(self.bot, f"INSERT INTO schedules(guild_id, time, content, embed, title, footer, thumbnail, image, color, num, channel_id, loop_integer, duration) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", ctx.guild.id, time, description, 0, "None", "None", "None", attachment, "None", len(datas)+1, channel.id, loop_integer, str(duration))
            ...
            while None in elements:
                await ctx.send(embed=discord.Embed(title=element_name, description=f"Now input a {element_name} for the embed, Type `None` to skip it."))
                msg = await self.bot.wait_for('message', check=lambda m: m.author.id == ctx.author.id and m.channel.id == ctx.channel.id, timeout=360.0)
                ...
                current_element += 1
            datas = await func.DataFetch(self.bot, 'all', 'schedules', ctx.guild.id)
            await func.DataUpdate(self.bot, f"INSERT INTO schedules(guild_id, time, content, embed, title, footer, thumbnail, image, color, num, channel_id, loop_integer, duration) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", ctx.guild.id, time, description, 1, elements[0], elements[1], elements[2], elements[3], elements[4], len(datas)+1, channel.id, loop_integer, str(duration))
            await ctx.send(embed=func.SuccessEmbed('Schedule Added!', "Schedule added successfully."))

    @add.command()
    async def embed(self, ctx, channel: discord.TextChannel, duration, *, description):
        try:
    ...
    async def overview(self, ctx):
        try:
            ...
            await ctx.send(embed=discord.Embed(title="Select a Schedule", description="Here's a list of all schedules with their trigger time. Select any one of them to see their embed/message."), view=DropdownView(self.bot, ctx, datas, 'overview'))
        except:
            print(traceback.format_exc())
    ...
    async def remove(self, ctx):
        ...
        if len(datas) == 0:
            return await ctx.send(embed=func.ErrorEmbed('Error', 'There are no active schedules.'))
        await ctx.send(embed=discord.Embed(title="Select a Schedule", description="Here's a list of all schedules with their trigger time. Select any one of them to see their embed/message and delete them."), view=DropdownView(self.bot, ctx, datas, 'remove'))
    ...
    async def message_edit(self, ctx, channel:discord.TextChannel = "None", * , message):
        datas = await func.DataFetch(self.bot, 'all', 'schedules', ctx.guild.id)
        await ctx.send(embed=discord.Embed(title="Select a schedule", description="Here's a list of all schedules with their trigger time. Select any one of them to see their embed/message and edit them."), view=DropdownView(self.bot, ctx, datas, 'edit:0', channel, message))
    ...
    async def embed_edit(self, ctx, channel:discord.TextChannel = "None", * , description):
        datas = await func.DataFetch(self.bot, 'all', 'schedules', ctx.guild.id)
        if len(datas) == 0:
            return await ctx.send(embed=func.ErrorEmbed('Error', 'There are no active schedules.'))
        await ctx.send(embed=discord.Embed(title="Select a Schedule", description="Here's a list of all schedules with their trigger time. Select any one of them to see their embed/message and edit them."), view=DropdownView(self.bot, ctx, datas, 'edit:1', channel, description))
