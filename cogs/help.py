...
class Help(Cog):
    ...
    async def help(self,ctx):
        e = discord.Embed(title=":notepad_spiral: Help Menu :notepad_spiral:", description="• Prefix is .\n•View more about each command using `.help <command-name>`, for Example: `.help schedule`", color=ctx.author.color)
        e.add_field(name="Commands", value="• `schedule`")
        await ctx.send(embed=e)

    @help.command()
    async def schedule(self, ctx):
        e = discord.Embed(title=":calendar: Schedule", description=f"Scheduler system commands.", color=ctx.author.color)
